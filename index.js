...
class ClickUpTimeTracker {
    ...
    try {
      ...
      if (!this.teamId) {
        ...
        if (teams.length > 0) {
          ...
          console.log(
            `Используется команда: ${teams[0].name} (ID: ${this.teamId})`.yellow
          );
        } else {
      ...
      console.error(
        "Ошибка получения записей времени:",
        error.response?.data || error.message
      );
      throw error;
    }
  ...
  async getTaskDetails(taskId) {
    try {
      ...
      console.error(
        `Ошибка получения деталей задачи ${taskId}:`,
        error.response?.data || error.message
      );
      return null;
    }
  ...
  calculateStatistics(timeEntries, totalTime) {
    const stats = {
      ...
      avgSessionTime:
        timeEntries.length > 0 ? totalTime / timeEntries.length : 0,
      workingDays: 0,
      weekdayTime: 0,
      weekendTime: 0,
    };
    ...
    stats.avgDayTime =
      stats.workingDays > 0 ? totalTime / stats.workingDays : 0;
  ...
  async generateMonthlyReport(year, month, options = {}) {
    try {
      ...
      console.log(
        `Период: ${startOfMonth.format("DD.MM.YYYY")} - ${endOfMonth.format(
          "DD.MM.YYYY"
        )}`.cyan
      );
      ...
      const report = {
        ...
        tasks: [],
        days: Object.values(dayGroups).sort((a, b) =>
        ...
        report.tasks.push({
          ...
          entries: taskGroup.entries.map((entry) => ({
            ...
            start: moment(parseInt(entry.start)).format("DD.MM.YYYY HH:mm"),
              ...
              ? moment(parseInt(entry.end)).format("DD.MM.YYYY HH:mm")
              : "В процессе",
          })),
        });
      }
      ...
      report.days.forEach((day) => {
        ...
        day.isWeekend =
          moment(day.date).day() === 0 || moment(day.date).day() === 6;
      });
  ...
  printReport(report, options = {}) {
    const showTasks =
      options.showTasks !== undefined
        ? options.showTasks
        : this.userPrefs.get("showTasks");
    const showDays =
      options.showDays !== undefined
        ? options.showDays
        : this.userPrefs.get("showDays");
    const showTimeEntries =
      options.showTimeEntries !== undefined
        ? options.showTimeEntries
        : this.userPrefs.get("showTimeEntries");
    const showCost =
      options.showCost !== undefined
        ? options.showCost
        : this.userPrefs.get("showCost");
    const showStatistics =
      options.showStatistics !== undefined
        ? options.showStatistics
        : this.userPrefs.get("showStatistics");

    console.log("\n" + "=".repeat(80).rainbow);
    console.log(
      `📊 ОТЧЕТ ПО ВРЕМЕНИ ЗА ${report.period.month}/${report.period.year}`.bold
        .white
    );
    console.log("=".repeat(80).rainbow);

    console.log(
      `👤 Пользователь: ${report.user.username} (${report.user.email})`.cyan
    );
    console.log(
      `📅 Период: ${report.period.start} - ${report.period.end}`.cyan
    );
    console.log(
      `⏱️  Общее время: ${report.summary.totalTimeFormatted}`.green.bold
    );

    if (showCost && report.summary.cost) {
      console.log(
        `� Стоимость: ${this.userPrefs.formatCost(
          report.summary.cost.grossCost
        )} (брутто)`.green.bold
      );
      console.log(
        `💸 К получению: ${this.userPrefs.formatCost(
          report.summary.cost.netCost
        )} (после налогов)`.green
      );
    }
    ...
    if (showStatistics && report.statistics) {
      ...
      console.log(
        `⏱️  Среднее время в день: ${this.formatDuration(stats.avgDayTime)}`
          .blue
      );
      console.log(
        `⌚ Среднее время на сессию: ${this.formatDuration(
          stats.avgSessionTime
        )}`.blue
      );
      console.log(
        `🏢 Время в будни: ${this.formatDuration(stats.weekdayTime)}`.blue
      );
      console.log(
        `🏖️  Время в выходные: ${this.formatDuration(stats.weekendTime)}`.blue
      );
    }
    ...
    if (showDays) {
      ...
      for (const day of report.days) {
        ...
        if (showCost && day.cost) {
          console.log(
            `   Стоимость: ${this.userPrefs.formatCost(day.cost.grossCost)}`
              .green
          );
        }

        console.log(
          `   Задач: ${day.tasks.length} (${day.tasks.join(", ")})`.blue
        );
      }
    }
      ...
      for (let i = 0; i < report.tasks.length; i++) {
        ...
        if (showCost && task.cost) {
          console.log(
            `   Стоимость: ${this.userPrefs.formatCost(
              task.cost.grossCost
            )} (${this.userPrefs.formatCost(task.cost.netCost)} чистыми)`.green
          );
        }
        ...
        if (showTimeEntries && task.entries.length > 0) {
          ...
          task.entries.forEach((entry, idx) => {
            let entryLine = `     ${idx + 1}. ${entry.durationFormatted} - ${
              entry.description
            }`.gray;
            if (showCost && entry.cost) {
              entryLine += ` (${this.userPrefs.formatCost(
                entry.cost.grossCost
              )})`.green;
            }
  ...
  async saveReport(report) {
    const fileName = `report_${report.period.year}_${report.period.month
      .toString()
      .padStart(2, "0")}.json`;
  ...
  async saveReportAsCSV(report) {
    const csvFileName = `report_${report.period.year}_${report.period.month
      .toString()
      .padStart(2, "0")}.csv`;
    ...
    let csvContent =
      "Задача,ID,Статус,Список,Время (часы),Время (форматированное),Количество записей,URL";
    if (showCost) {
    ...
    for (const task of report.tasks) {
      ...
      if (showCost && task.cost) {
        row += `,"${task.cost.grossCost.toFixed(
          2
        )}","${task.cost.netCost.toFixed(2)}","${task.cost.currency}"`;
      }
  ...
  async saveDailyReportAsCSV(report) {
    const csvFileName = `daily_report_${
      report.period.year
    }_${report.period.month.toString().padStart(2, "0")}.csv`;
    const csvFilePath = path.join(this.reportsDir, csvFileName);

    const showCost = this.userPrefs.get("showCost");

    let csvContent =
      "Дата,День недели,Время (часы),Время (форматированное),Количество задач,Задачи";
    if (showCost) {
      csvContent += ",Стоимость (брутто),Стоимость (нетто),Валюта";
    }
    csvContent += "\n";
    ...
    for (const day of report.days) {
      ...
      if (showCost && day.cost) {
        row += `,"${day.cost.grossCost.toFixed(2)}","${day.cost.netCost.toFixed(
          2
        )}","${day.cost.currency}"`;
      }
  ...
  try {
    ...
    if (args.length === 0) {
      ...
      if (isNaN(year) || isNaN(month) || month < 1 || month > 12) {
        console.error(
          "Неверный формат. Использование: node index.js [год] [месяц]".red
        );
        process.exit(1);
      }
      ...
      console.log(
        "  node index.js daily        - отчет по дням за текущий месяц"
      );
