...
class ClickUpTimeTrackerDemo {
  ...
  async generateDemoReport(year, month) {
    console.log(
      `🎭 ДЕМО РЕЖИМ - Генерация отчета за ${month}/${year}...`.magenta
    );
    ...
    const sortedTasks = Object.values(taskGroups).sort(
    ...
    console.log(
      "\n🎭 Это демо данные! Для реальных данных настройте .env файл с вашим ClickUp токеном."
        .magenta
    );
  ...
  printReport(report) {
    console.log("\n" + "=".repeat(80).rainbow);
    console.log(
      `📊 ДЕМО ОТЧЕТ ПО ВРЕМЕНИ ЗА ${report.period.month}/${report.period.year}`
        .bold.white
    );
  ...
  async saveReport(report, prefix = "") {
    const fileName = `${prefix ? prefix + "_" : ""}report_${
      report.period.year
    }_${report.period.month.toString().padStart(2, "0")}.json`;
