...
async function setupConfig() {
  ...
  const teamId = await question(
    "Введите Team ID (оставьте пустым для автоопределения): "
  );

  console.log("\n💰 Финансовые настройки:".yellow);
  const hourlyRate =
    (await question("Почасовая ставка (по умолчанию 25): ")) || "25";
  const currency =
    (await question("Валюта [USD/RUB/EUR] (по умолчанию USD): ")) || "USD";
  const taxRate =
    (await question(
      "Налоговая ставка в долях (например, 0.2 для 20%, по умолчанию 0.2): "
    )) || "0.2";
...
async function configureDisplaySettings() {
  try {
    ...
    console.log("\n💰 Финансовые настройки:".yellow);
    const newRate = await question(
      `Почасовая ставка (текущая: ${current.hourlyRate}): `
    );
    if (newRate) userPrefs.set("hourlyRate", parseFloat(newRate));

    const newCurrency = await question(
      `Валюта (текущая: ${current.currency}): `
    );
    if (newCurrency) userPrefs.set("currency", newCurrency.toUpperCase());

    const newTaxRate = await question(
      `Налоговая ставка в долях (текущая: ${current.taxRate}): `
    );
    if (newTaxRate) userPrefs.set("taxRate", parseFloat(newTaxRate));

    console.log("\n📊 Настройки отображения:".yellow);

    const showCost = await question(
      `Показывать стоимость? [y/n] (текущее: ${current.showCost ? "y" : "n"}): `
    );
    if (showCost) userPrefs.set("showCost", showCost.toLowerCase() === "y");

    const showTimeEntries = await question(
      `Показывать отдельные записи времени? [y/n] (текущее: ${
        current.showTimeEntries ? "y" : "n"
      }): `
    );
    if (showTimeEntries)
      userPrefs.set("showTimeEntries", showTimeEntries.toLowerCase() === "y");

    const showDays = await question(
      `Показывать разбивку по дням? [y/n] (текущее: ${
        current.showDays ? "y" : "n"
      }): `
    );
    if (showDays) userPrefs.set("showDays", showDays.toLowerCase() === "y");

    const showTasks = await question(
      `Показывать задачи? [y/n] (текущее: ${current.showTasks ? "y" : "n"}): `
    );
    if (showTasks) userPrefs.set("showTasks", showTasks.toLowerCase() === "y");

    const showStatistics = await question(
      `Показывать статистику? [y/n] (текущее: ${
        current.showStatistics ? "y" : "n"
      }): `
    );
    if (showStatistics)
      userPrefs.set("showStatistics", showStatistics.toLowerCase() === "y");
    ...
    const sortChoice = await question(
      `Выберите сортировку (1-5, текущая: ${current.sortBy}): `
    );
    const sortOptions = [
      "time_desc",
      "time_asc",
...
Ваш выбор (1-8): `);
    ...
    switch (choice.trim()) {
      ...
      case "4":
        const yearDaily =
          (await question("Введите год (по умолчанию текущий): ")) ||
          moment().year();
        const monthDaily =
          (await question("Введите месяц (по умолчанию текущий): ")) ||
          moment().month() + 1;
