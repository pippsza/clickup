...
class UserPreferences {
  constructor() {
    this.preferencesFile = path.join(
      __dirname,
      config.interactive.preferencesFile
    );
    this.defaultPreferences = {
      hourlyRate:
        parseFloat(process.env.HOURLY_RATE) || config.billing.hourlyRate,
      currency: process.env.CURRENCY || config.billing.currency,
      taxRate: parseFloat(process.env.TAX_RATE) || config.billing.taxRate,
      showTimeEntries:
        process.env.SHOW_TIME_ENTRIES === "true" ||
        config.output.showTimeEntries,
      showDays:
        process.env.SHOW_DAILY_BREAKDOWN === "true" || config.output.showDays,
